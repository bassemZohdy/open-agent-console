...
function credential(model: ModelRecord): string {
  ...
  if (!apiKey) throw new Error(`Credential environment variable ${model.apiKeyEnv} is not configured`);
  return apiKey;
}
...
export async function testModelConnection(model: ModelRecord): Promise<{ latencyMs: number }> {
  const started = Date.now();
  const chatModel = createChatModel(model, { maxTokens: Math.min(model.maxTokens ?? 16, 16), temperature: 0 });
  await chatModel.invoke('Reply with exactly OK.', { timeout: model.timeoutMs });
